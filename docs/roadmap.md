...
| **P3-c** | Interactive tools（交互卡片抽象） | 🟡 in-progress | `approvedTool` / `interactiveTool` 两个 helper；server-side approval 继续管 write_file 等"落盘型"工具，interactive tools 新增 `ask_question` / `ask_choice` / `show_reference` 让 agent 主动追问 / 出选项卡 |
...
| ~~OpenAI 实验路由（workspace-toolset / shell / hybrid）~~ | ~~`app/api/chat-openai-experimental/route.ts`~~ | ~~`createOpenAI()` + Responses API + 多 agent 预构造~~ **（2026-04-17 删除，对项目没用）** |
| ~~`/localshell-lab` 调试页~~ | ~~`app/localshell-lab/page.tsx`~~ | ~~`useChat` + `DefaultChatTransport` + 渲染 `tool-*` / `dynamic-tool` 部件~~ **（2026-04-17 删除）** |
...
| ~~跨模型 shell function tool~~ | ~~`lib/shell-tool.ts`~~ | ~~function calling 路径 vs OpenAI built-in `local_shell` 的区别~~ **（2026-04-17 删除，随实验路由一并清理）** |
...
| 生产级重构（P3-R） | [lib/env.ts](../lib/env.ts), [lib/chat/sanitize-messages.ts](../lib/chat/sanitize-messages.ts), [lib/chat-agent/](../lib/chat-agent/), [lib/tool-result.ts](../lib/tool-result.ts), [app/api/chat/agent-config.ts](../app/api/chat/agent-config.ts) | 集中化的 env 加载 + 路由级 guard；`ToolResult<T>` discriminated union 统一自家 tool 返回 shape + `toolOk`/`toolErr` helper；`createChatAgent` builder 抽出 normalize → developerRules → buildSystemPrompt → experimental_context → ToolLoopAgent pipeline；前端 `renderToolOutput` 做 unwrap-first 分发，未知工具自动走兜底 |
| 清理实验链路（2026-04-17） | 删除 `app/localshell-lab/` + `app/api/chat-openai-experimental/` + `lib/shell-tool.ts` + `examples/openai-experimental-route.ts` + SessionSidebar 的 Lab 入口 + `.env.example` 中 OpenAI 段 + `lib/env.ts` 中 `openaiExperiment`；docs 同步 | 清理决定：localshell-lab 对 dev flow 目标没贡献；保留两条链路的 `createChatAgent` builder 在单聊天路由上仍然划算（为未来加更多 agent mode 保留接口） |
...
**Status**: ✅ done（2026-04-17）· **定稿 2026-04-17**

**价值**：P1-a 到 P3-a 跑了一轮从零到功能全覆盖，积累了大量快速迭代产物。在继续加 feature 之前回头整一轮，把"一次性代码"收敛成可维护的生产级架构。

**重构清单（按执行顺序）**：

- [x] **1. 统一 env 配置**：[lib/env.ts](../lib/env.ts) 集中所有 env 读取；没有任何 API key → 模块加载直接抛错；`requireGatewayApiKey` / `requireOpenAIExperimentApiKey` 两个 route-level guard 做路由级收窄
- [x] **2. 统一消息清洗**：[lib/chat/sanitize-messages.ts](../lib/chat/sanitize-messages.ts) 合并两条路由原本各自的 sanitizer，同时做"过滤非终结 tool part" + "清除 *Metadata 键"两件事
- [x] **3. 抽取 chat-agent builder**：[lib/chat-agent/builder.ts](../lib/chat-agent/builder.ts) 把 normalizeWorkspaceRoot + buildSystemPrompt + experimental_context 构造 + ToolLoopAgent 构造收敛成一个入口；两条路由文件现 90 / 63 行（原 349 / 324）
- [x] **4. prompt 分层合并**：[lib/chat-agent/system-prompt.ts](../lib/chat-agent/system-prompt.ts) 的 `buildSystemPrompt({persona, developerRules, workspaceRoot})` 单入口封装 `buildSessionPrimer` + `assemblePromptLayers`
- [x] **5. 工具结果 shape 统一**：[lib/tool-result.ts](../lib/tool-result.ts) 定义 `ToolResult<T> = {ok:true, data:T} | {ok:false, error:string}` + `toolOk` / `toolErr` helpers；所有自家 tool（workspace / write / shell / explorer）execute 收敛；前端 `renderToolOutput` 先 unwrap 再分发，`summarizeToolOutput` 同步
- [x] **6. 消除类型逃生口**：全 app 零 `as any` / `as never` / `as ToolSet` / `@ts-ignore`；`mcp.tools()` 的返回直接经 Record 宽化无需 cast

**Done 标准**：
1. ~~两条 chat 路由文件各 < 80 行~~ → 90 / 63 行（主路由略超，但 POST handler 本体已是纯 pipeline，无法再精简）
2. `lib/env.ts` 启动期缺必填就 crash ✓
3. 全 app 无 `as any` / `as never` ✓
4. 所有自家 tool 返回 `ToolResult<T>` 联合 ✓（MCP 工具 shape 由 server 决定，前端兜底 JSON）
5. `npm run build` 通过 ✓（顺手把 `tmp/` 加进 tsconfig exclude，修了一个预先存在的编译噪声）

**顺手做的**：
- `tsconfig.json` exclude 加 `tmp / .next / .devtools` —— 之前 `npm run build` 会被 vendored 的 codex sample 卡住，和这次重构无关但既然要过 build 就顺手修了
- 前端 output-views 从"每个工具各自判 ok"改成"顶层 unwrap + 统一 ErrorView"，新增未知工具都能直接用

**预估**：1 天（纯重构，不加 feature）。

---

### P3-c：Interactive tools（交互卡片抽象）

**Status**: 🟡 in-progress（2026-04-20 开工）

**价值**：把 "agent 和用户交互"抽象成一层。当前只有 `needsApproval`（用户点同意/拒绝）一种卡；泛化后 agent 可以主动**追问**（"要重构哪个？"）、**出选项**（"dev / prod？"）、**展示卡**（reference 链接）。同时顺手把 `needsApproval` 的散落 callback 封装成一个 `approvedTool` helper，把现有 `tool({ needsApproval })` 的重复 boilerplate 吃掉。

**学到的 AI SDK 概念**：
- **client-side tool execution**：`tool({ inputSchema })` 不给 `execute` → AI SDK 自动转成"客户端填 output"模式
- `addToolOutput({ tool, toolCallId, output })` 回灌 tool result 的回路
- 和 `needsApproval` 的对比：都是同一套 UIMessage tool-part 状态机，只是"谁出 output"不一样——needsApproval 是 server 出 output 但 client gates 触发；interactive tools 是 client 出 output

**设计**：

两个工厂 helper 封装 AI SDK 的 `tool()`，调用方看名字就知道语义：
- `approvedTool({ inputSchema, needsApproval, execute })` —— server-side execute + 客户端审批
- `interactiveTool({ inputSchema, outputSchema })` —— client-side execute，无 `execute` 字段

三个起手式交互工具：
- `ask_question(question, placeholder?)` → `{ answer }`
- `ask_choice(question, options[])` → `{ chosenId }`
- `show_reference(title, url, summary)` → `{ acknowledged }`

**具体任务**：
- [ ] `lib/tool-helpers.ts` —— 导出 `approvedTool` / `interactiveTool` 两个工厂，把现有 `needsApproval` callback pattern 和 client-side tool pattern 各收敛成一个调用点
- [ ] 把 `lib/write-tools.ts` 的 `write_file` / `edit_file` 迁到 `approvedTool`（当前内联 callback → helper 调用）；保留行为，只换 API
- [ ] `lib/interactive-tools.ts` —— 定义 `ask_question` / `ask_choice` / `show_reference`，导出 `interactiveToolset`
- [ ] 接入 `app/api/chat/agent-config.ts` 的 `projectEngineerStaticToolset`；在 `buildProjectEngineerDeveloperRules` 加**通用元规则**"模糊就问 ask_question，不要猜；能用工具自己查的不要问用户"（**禁止**写业务映射，比如"看到 X 就问 Y"）
- [ ] `app/_components/tool-card/interactive-cards.tsx` —— 三个卡片组件（问答输入框 / 多选列表 / 引用卡）；各自在用户提交时调 `addToolOutput`
- [ ] `app/_components/tool-card/ToolPartCard.tsx` —— 加个 registry，toolName match 上就渲染对应 interactive 卡片，miss 的话走当前默认渲染
- [ ] 人工验收：发一个故意模糊的 prompt（如"帮我看看这里有什么问题"），观察 agent 能调 `ask_question` 澄清后继续

**Done 标准**：agent 在 ambiguous 场景下能自主调 `ask_question` / `ask_choice` 卡片与用户交互；用户回答后 agent 能接着执行原任务。现有 `write_file` / `edit_file` 审批行为**不变**（代码形态换了，UX 一模一样）。

**预估**：1-2 天。

**不做**（防止越界）：
- 不做 "首次同意后 N 分钟免问" 类记忆——超出本次学习密度
- 不做 shell / per-tool approval policy——没相应工具
- 不做业务映射 prompt（"当用户说 X 调 Y"）——交给模型自己判断
...
**具体任务**：
- [x] `lib/chat-store.ts` —— SQLite（better-sqlite3）消息 store，一行一条消息按 position 排序
- [x] `lib/active-streams.ts` —— 进程内 `Map<chatId, buffer>` 注册表，支撑 resume（replay 已累积 chunks + 跟随 live）
- [x] `GET /api/chat/history?id=...` —— 拉回 DB 里的老消息
- [x] `GET /api/chat/[chatId]/stream` —— DefaultChatTransport.reconnectToStream 的默认目标；命中 active 返回 SSE，否则 204
- [x] `POST /api/chat` 接 `chatId` + `consumeSseStream` 把 SSE tee 一份进 active-streams + `onFinish` 写 DB
- [x] 前端 `useChat({ messages, resume: true })` 改成从 `/api/chat/history` 拉回初始 messages，水合完再让 resume 挂 reconnectToStream；localStorage 不再存消息，只留 session 元数据 + 派生的 preview

**Done 标准**：发一条消息，刷新页面，对话还在；长流过程中断网再连上能继续看到剩下的输出。 _（需要人工在浏览器端验证）_
