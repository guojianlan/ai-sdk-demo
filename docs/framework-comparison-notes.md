...
### 2.3 只读 shell 的安全模型是清晰的

[`lib/shell-tool.ts`](../lib/shell-tool.ts) 不是简单地把任意 shell 暴露给模型，而是：

- 限制 binary 白名单
- 限制 git 子命令白名单
- 对 `sh -c` 做危险操作符检测
- 把执行目录继续限制在 workspace 内

这说明项目已经意识到：shell 不是“多一个工具”，而是一个高风险入口。
...
- [`app/api/chat/route.ts`](../app/api/chat/route.ts) 对悬空 tool parts 做清洗，避免残缺上下文污染后续请求

这说明项目不是没考虑 context，只是还没做到“正式 compaction pipeline”。

### 2.6 Plan 模式是独立链路，不和执行链路缠死
...
当前仓库直接在 Node 进程里通过 [`lib/workspaces.ts`](../lib/workspaces.ts) 和 [`lib/shell-tool.ts`](../lib/shell-tool.ts) 操作本地路径。
...
### `lib/workspaces.ts` 和 `lib/shell-tool.ts`
